...
namespace UbiArt.CRC {
    public static class CrcHelper
    {
        ...
        /// </summary>
        public static uint ToUInt32(byte[] hash, bool reverse=false)
        {
			if (!reverse) {
				return FromBigToUInt32(hash, 4);
			} else {
				byte[] lol = BitConverter.GetBytes(FromBigToUInt32(hash, 4));
				Array.Reverse(lol);
				return BitConverter.ToUInt32(lol, 0);
			}
        }
