...
namespace GXTConvert.Conversion
{
    public static class PostProcessing
    {
        ...
        private static int GetTilePixelOffset(int t, int x, int y, int width, uint bpp)
        {
            return (GetTilePixelIndex(t, x, y, width) * ((int)bpp / 8));
        }

        public static byte[] UntileTexture(byte[] pixelData, int width, int height, uint bpp)
        {
            ...
            for (int y = 0; y < height; y += 8)
            {
                for (int x = 0; x < width; x += 8)
                {
                    for (int t = 0; t < (8 * 8); t++)
                    {
                        int pixelOffset = GetTilePixelOffset(t, x, y, width, bpp);
        ...
        public static byte[] UnswizzleTexture(byte[] pixelData, int width, int height, uint bpp)
        {
