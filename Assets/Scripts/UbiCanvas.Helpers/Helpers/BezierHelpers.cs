...
namespace UbiCanvas.Helpers {
	public static class BezierHelpers {
		...
		public static Vec2d[] GetControlPoints(Vec2d point0, Vec2d point1, Vec2d normal0, Vec2d normal1, bool flip = false) {
			var N01 = (float)(point0 - point1).Magnitude * 0.5f;
			if (flip) N01 = -N01;
			return new Vec2d[] {
				point0,
				point0 + new Vec2d(-normal0.y, normal0.x) * N01,
				point1 - new Vec2d(-normal1.y, normal1.x) * N01,
				point1
			};
		}
		public static Vec2d[] GetControlPoints<T>(T point0, T point1, Func<T, Vec2d> pointFunc, Func<T, Vec2d> normalFunc, bool flip = false) {
